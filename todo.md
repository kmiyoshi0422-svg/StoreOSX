...
## 業者メール通知／予定日集中警告／空き状況カレンダー（ユーザー指示）
- [ ] 既存の業者メール、通知送信、施工工程データを棚卸し
- [ ] 業者の新規割当・変更時に本人へメール通知する処理を実装
- [ ] メール未登録・送信失敗時も割当保存を維持し、結果を管理者・社員へ表示
- [ ] 同一予定日の案件集中件数を判定するAPIを実装
- [ ] 管理者・社員の設定画面に予定日重複警告を追加
- [ ] 業者別の施工スケジュール・空き状況APIを実装
- [ ] 業者選択時に月間カレンダー形式で予定件数と案件を確認できるUIを追加
- [ ] partnerの設定・他業者スケジュール取得を拒否する権限テストを追加
- [ ] 通知・集中判定・カレンダー表示のテストと実DBスモークを実行
- [ ] 全Vitest・型チェック・本番ビルドを完走
