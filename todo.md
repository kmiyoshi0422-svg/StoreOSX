...
## 追加機能（v16）担当者別ワークロード可視化

- [ ] サーバー: workload.list ルーターを追加（期間内の各担当者について現調/工事件数・稼働日数・総移動距離・チーム別件数を集計）
- [ ] /workload ページ + サイドバーメニューを追加（横棒グラフ + KPIカード + テーブル）
- [ ] 期間プリセット（今週/来週/今月/14日先）で切替可能
- [ ] 偏り警告：最大件数と最小件数の差が一定以上なら「偏りあり」バッジ
- [ ] vitest 追加（集計ロジック・空データ処理・偏り判定）
