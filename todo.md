...
## 横断工程表の視認性改善（ユーザー指示）
- [x] バーの高さを拡大（18px→24px）し、テキストを読みやすく
- [x] 行の縞模様（ストライプ）追加で行の区別を明確化
- [x] 日付ヘッダーのフォントサイズ・コントラスト改善
- [x] バーのカラーをより鮮やかに（opacity増加）、ボーダー追加
- [x] 左カラム（業者名/案件名）の幅を拡大し、情報量を増やす
- [x] 進捗ステータスバッジをバー内に表示（予定/進行中/完了）
- [x] 行間のスペーシング改善
- [x] 凡例をより分かりやすく改善

## AI所感が生成されない問題の修正（ユーザー報告バグ）
...
## 案件ステータス不整合の修正（ユーザー報告バグ）
- [x] progressStageが未対応のままstatusが進んでいる4件のDB修正（statusToStageで正しいstageに修正）
- [x] cases.update APIでstatusのみ変更時にprogressStageの連動が漏れるケースの防止強化
- [x] テストデータ（テスト店舗・テスト案件・テスト協力業者）の削除
...
## 協力業者の現調報告書写真アップロード機能改善（ユーザー指示）
- [x] partner権限で写真アップロードAPIが利用可能であることを確認（既に対応済み）
- [x] 複数枚同時アップロード時の進捗バー表示を追加
- [x] 個別ファイル失敗時もスキップして残りを継続する耐障害性を追加
- [x] ボタンテキストを「ファイルから追加（複数可）」に変更して複数選択可能であることを明示

## 報告書画面にサムネイルグリッド閲覧モードを追加（ユーザー指示）
- [x] グリッド表示/カード表示のトグルボタンを追加
- [x] グリッド表示モード：シンプルなサムネイルグリッド（3〜4列）でクリックで拡大→スライド閲覧
- [x] グリッド表示時は写真番号・区分バッジのみオーバーレイ表示

## 野口さん（partner）の権限を社員（user）と同等にする（ユーザー指示）
- [x] 野口さんのroleをpartnerからuserに変更（DB更新）

## 経費データの入力者・入力日時追跡機能（ユーザー指示）
- [x] DBスキーマ: expensesテーブルにcreatedByName（入力者名）カラムを追加
- [x] サーバー: 経費登録時にuploadedBy（ユーザーID）とcreatedByName（ユーザー名）を自動記録
- [x] サーバー: 経費更新時にupdatedByName（更新者名）を記録
- [x] フロントエンド: 経費一覧に「入力者」「入力日時」カラムを表示
- [x] フロントエンド: 案件詳細の経費タブにも入力者・日時を表示
- [x] 経費データCSVエクスポート機能（入力者・日時含む全データをダウンロード可能に）

## 経費一覧画面にフィルター機能を追加（ユーザー指示）
- [x] 入力者（createdByName）で絞り込めるフィルターを追加
- [x] 期間（日付範囲）で絞り込めるフィルターを追加
- [x] 区分（category）で絞り込めるフィルターを追加
- [x] フィルターUIを経費一覧（立替者別経費ページ）に実装

## 経費機能拡張（ユーザー指示）
- [x] 経費明細一覧ページ新設（全経費を1件ずつ一覧表示、インライン編集・削除対応）
- [x] 経費の月次推移グラフ（過去6ヶ月の推移を棒グラフで可視化）
- [x] 既存経費データの入力者名補完（uploadedByからcreatedByNameを一括埋め）

## 経費承認ワークフロー・レシートプレビュー・予算アラート（ユーザー指示）
- [x] DB: expensesテーブルにapprovalStatus（pending/approved/rejected）、approvedBy、approvedAt、rejectionReasonカラム追加
- [x] DB: casesテーブルにexpenseBudget（経費予算上限）カラム追加
- [x] サーバー: 経費申請プロシージャ（レシート画像付きで経費を申請）
- [x] サーバー: 承認/却下プロシージャ（管理者のみ）
- [x] サーバー: 案件別予算超過チェック＆通知ロジック
- [x] フロントエンド: 経費申請フォーム（金額・区分・レシート画像アップロード・案件選択）
- [x] フロントエンド: 承認管理画面（未承認一覧・承認/却下ボタン）
- [x] フロントエンド: 経費明細一覧にレシート画像サムネイル＆拡大プレビュー
- [x] フロントエンド: 案件詳細に予算アラート表示（超過時に警告）

## 経費通知・月次レポート・予算UI（ユーザー指示）
- [x] 経費申請時に管理者へ自動通知（notifyOwnerで新規申請を知らせる）
- [x] 月次経費レポート自動生成（Heartbeat定期タスクで月末に集計→オーナー通知）
- [x] 案件詳細から予算上限を設定するUI（expenseBudgetフィールドの編集）

## 管理者・社員・協力業者の3権限実装（ユーザー指示）
- [x] サーバー側: partner権限のユーザーにはcases.getレスポンスから金額情報を除外
- [x] サーバー側: partner権限は担当案件のみ閲覧可能に制限
- [x] サーバー側: partner権限は経費明細を自分の分のみに制限
- [x] フロントエンド: partnerユーザーに見積金額・予算・原価関連UIを非表示
- [x] フロントエンド: partnerユーザーのサイドバーメニューを制限（経費承認・予算設定等を非表示）
- [x] partnerユーザーが担当案件のステータス変更（施工中→完了等）は可能にする

## 報告書「作成完了」＋夜間自動PDF生成＋管理者通知（ユーザー指示）
- [x] DB: casesテーブルにreportStatus（draft/completed）・reportCompletedAt・reportPdfUrlカラム追加
- [x] フロントエンド: 報告書画面に「作成完了」ボタン追加（確認ダイアログ付き）
- [x] サーバー: 報告書完了APIプロシージャ（reportStatusをcompletedに更新）
- [x] サーバー: 夜間Heartbeatタスク（2〜5時）で完了済み＆未生成の報告書PDFをサーバーサイド生成
- [x] サーバー: PDF生成後に管理者全員にアプリ内通知（案件名＋PDFリンク付き）

## 完了済み報告書一覧ページ（ユーザー指示）
- [x] サーバー: 完了済み報告書一覧取得プロシージャ追加
- [x] フロントエンド: 完了済み報告書一覧ページ新設（テーブル表示・PDFリンク・フィルター）
- [x] サイドバーにメニュー追加・ルート登録

## 報告書差し戻し機能（ユーザー指示）
- [x] DB: casesテーブルにreportRejectComment・reportRejectedAt・reportRejectedByカラム追加
- [x] サーバー: 差し戻しAPIプロシージャ（reportStatusをdraftに戻す＋コメント記録）
- [x] フロントエンド: 完了報告書一覧に差し戻しボタン＋コメント入力ダイアログ追加
- [x] フロントエンド: 報告書画面に差し戻しコメント表示（差し戻された場合）

## 報告書PDFダウンロード高速化（ユーザー指示）
- [x] 画像の並列fetch（inlineImages最適化）
- [x] レンダリング解像度の最適化（scale調整）
- [x] ページ処理の効率化（不要な再レンダリング削減）
- [x] 進捗表示の改善（ページ単位の進捗バー）

## GitHub PR #2 店舗マスタ紐付け機能の安全統合（ユーザー指示）
- [x] 現行ブランチとGitHub main・マージコミットa7ca514の対象2ファイル差分を確認
- [x] CaseDetail.tsxへ店舗マスタ紐付けUIを既存の最新変更を保持して統合
- [x] StoreMasterLinkPanel.tsxを取り込み、検索・選択・新規登録・変更・解除を実装
- [x] cases.updateのstoreId更新とstoreMaster.list/create/get/pastCases/pastPhotos API整合を確認
- [x] 管理側のみ表示・partner非表示・紐付け後に店舗履歴タブを開く挙動を確認
- [x] 関連Vitest、pnpm check、本番ビルドを実行して失敗を切り分け
- [x] 既存公開ドメインへ再公開し、配信中の案件詳細に対象文言が含まれることを確認

## StoreOSX 店舗マスタ紐付け機能まとめスライド（ユーザー指示）
- [x] GitHubコネクタ再認証と手動適用の安全手順を整理
- [x] 店舗マスタ検索・紐付け・履歴表示のローカル動作テスト手順を整理
- [x] 店舗マスタ紐付け機能・API整合・権限・ビルド状態をスライド化
- [x] スライド内容と視認性を確認して納品

## storeMaster.list 500エラーのDB整合調査（ユーザー指示）
- [x] Drizzleのstore_master定義・storeMaster.list実装・マイグレーションを照合
- [x] 実DBを読み取り専用SQLで確認し、不足していたstore_master・survey_skip_logs・索引を特定
- [x] 既存0028マイグレーションと同一構造で欠落テーブル・索引を作成
- [x] storeMaster.list実DBスモークテストと案件詳細画面を再検証

## 写真アップロード後に報告書を出力できない緊急不具合（ユーザー報告）
- [x] ブラウザ・API・本番ログから失敗箇所を特定
- [x] 写真読み込みまたはPDF生成処理の不具合を最小変更で修正
- [x] 写真付き報告書の生成・表示・PDFダウンロードを回帰テスト
- [x] 型チェック・全Vitest・本番ビルドを完走
- [x] 既存公開ドメインへ反映し、結果を報告

## ダッシュボード実データ連携・エクスポート・ドリルダウン（ユーザー指示）
- [x] 現行ダッシュボードの全ウィジェット・KPI・権限制御・データ取得元を棚卸し
- [x] ダッシュボード向け集計APIを追加し、各ウィジェットを実データへ統一
- [x] 現在の表示条件を反映したCSVエクスポートを追加
...
- [x] 管理者画面でPDF・CSVボタン・クリック可能KPIカード・実データグラフを最終確認
- [x] GitHub接続復旧後に既存公開ドメインへ反映
...
- [x] partnerログイン時に専用ダッシュボードを表示
- [x] 担当案件一覧（ステータス・進捗・緊急度表示）
- [x] 案件ステータス別サマリーカード
- [x] 直近のアクション（報告書作成・写真アップ等）へのクイックリンク

## 緊急更新：過去資料閲覧・店舗情報タブ（2026-09-08）
- [x] 過去資料が開けない再現条件と原因を特定する
- [x] 過去資料の権限・URL解決・表示処理を修正する
- [x] 店舗一覧から開ける店舗詳細ページを追加する
- [x] 店舗詳細を基本情報・店舗固有情報・過去案件/資料・設備台帳のタブ構成にする
- [x] 店舗基本情報を追記・編集できるようにする
- [x] 既存の設備台帳入力を店舗詳細へ統合する
- [x] 未紐付け案件への安全な作成・再照合導線を追加する
- [x] TypeScript型チェックを実行する（PASS）
- [x] 関連テストを実行する（資料URLテスト5件PASS、全体278件PASS・DB未接続由来12件失敗）
- [x] 本番ビルドを実行する（PASS）
- [x] GitHubへ反映し、公開環境を確認する
> 既存データを保持し、DBスキーマ変更なしで実装。

## 性能改善 提案43・44（2026-09-10）

### 提案44：店舗一覧集計をDB側へ移行
- [x] 店舗別集計をSQLのGROUP BY・条件付き集計へ移す
- [x] 最新案件のステータス・進捗をDB側で取得する
- [x] 店舗コード優先・店舗名フォールバックの現行互換性を維持する
- [x] 集約SQLの回帰テストを追加する（4件PASS）

### 提案43：案件詳細のタブ単位コード分割
- [x] 案件詳細の共通型・共通依存を整理する
- [x] 重い8タブを個別モジュールへ分離する
- [x] React.lazy・Suspenseで選択時のみ読み込む
- [x] 管理者・社員・協力業者の表示条件を維持する
- [x] CaseDetail初期チャンクの削減量を測定する（198,053→81,300 bytes）

### 共通検証・反映
- [x] TypeScript型チェックを実行する（PASS）
- [x] 関連テストを実行する（店舗集約4件PASS）
- [x] 全体テストを実行する（283件PASS、DB・ストレージ未接続由来12件失敗）
- [x] 本番ビルドを実行する（PASS）
- [x] GitHub mainへ反映する（dc1db44）
- [x] 公開環境への反映状況を確認する

## PDF履歴・ダッシュボード期間フィルター・店舗マスタ一括紐付け（ユーザー指示）
- [x] PDF生成履歴テーブルとマイグレーションを追加
- [x] PDF生成完了時にファイル・種別・案件・生成者・生成日時を履歴保存
- [x] PDF履歴一覧ページと再ダウンロード機能を追加
- [x] PDF履歴は管理者・社員のみ閲覧可能にする
- [x] ダッシュボード集計APIへ開始日・終了日フィルターを追加
- [x] ダッシュボード画面へ期間プリセット・任意日付範囲フィルターを追加
- [x] PDF/CSVエクスポートに選択期間を反映
- [x] 店舗コード優先・店舗名フォールバックの一括紐付け候補APIを追加
- [x] 一括紐付けのプレビュー・選択・実行UIを店舗一覧へ追加
...
## 依頼日14日経過案件・施工予定週表示（ユーザー指示）
- [x] 依頼日・施工予定日・工程データと既存KPIアラートを棚卸し
- [x] 依頼日から14日以上経過した未完了案件の判定をAPIへ追加
- [x] 設定済み施工予定日とその週の月曜〜日曜範囲をAPIで返す
- [x] ダッシュボードへ対象案件一覧と施工予定日・未設定表示を追加
- [x] 案件詳細への遷移とpartner担当案件制限を維持
- [x] Vitest・型チェック・本番ビルド・実データ・画面を検証
- [x] StoreOSX mainと既存公開ドメインへ反映（07bab67、公開バンドル検証済み）

## 経過案件を3か月以上・1か月以上・漏電関係へ区分（ユーザー指示）
- [x] 現行の14日経過判定と実案件データの漏電関連項目を棚卸し
- [x] 3か月以上、1か月以上3か月未満、漏電関係の判定ロジックを追加
- [x] 施工予定日・予定週・未設定表示とpartner担当案件制限を維持
- [x] ダッシュボードを3区分の切替表示へ変更
- [x] Vitest・型チェック・実DBスモーク・本番ビルドを完走
- [x] StoreOSX mainと既存公開ドメインへ反映（80b4504、公開バンドル検証済み）

## 未設定案件から施工予定日・施工業者を設定（ユーザー指示）
- [x] 施工予定日・施工業者・横断工程表・権限の既存データ構造を棚卸し
- [x] 管理者・社員限定の施工予定日・施工業者保存APIを実装
- [x] partnerによるAPI操作を拒否する権限テストを追加
- [x] 要対応案件の未設定行から予定日・施工業者を選択できるUIを追加
- [x] 保存後に予定週と業者表示を即時更新
- [x] Vitest・型チェック・実DBスモーク・本番ビルドを完走
- [x] StoreOSX mainと既存公開ドメインへ反映（d68f5da、公開バンドル検証済み）

## Zapierクラウド保存・ファイル台帳連携（ユーザー指示）
- [x] 資料アップロード後にZapier Catch Hookへ安全なファイル情報を送信
- [x] StoreOSX内の資料DB記録とZapier連携状態を分離して追跡
- [x] 失敗時の手動再送と連携状態表示を資料DB庫へ追加
- [x] Zapier完了コールバックをイベント固有トークンで保護
- [x] Zapier Tablesに「StoreOSX ファイル台帳」を作成
- [x] Google Drive保存→Zapier Tables記録のZap構成は、Zapier側の外部設定作業として別タスクへ移管（StoreOSX側の送信・追跡・再送機能は実装済み）
- [x] 型チェック・関連テスト・外部依存不要の全テスト・本番ビルドを実行
- [x] GitHubへ反映し、公開環境でZapier連携UIの配信を検証

## 施工予定・業者の変更解除／業者通知／一括設定（ユーザー指示）
- [x] 既存の施工割当・工程・partner通知・既読管理を棚卸し
- [x] 管理者・社員限定の変更・解除APIを実装
- [x] 複数未設定案件への一括予定日・業者設定APIを実装
- [x] 業者割当時に担当業者向け通知を作成し、本人だけ取得・既読化できるAPIを実装
- [x] 設定済み行へ変更・解除UIを追加
- [x] 未設定案件の複数選択・一括設定UIを追加
- [x] 協力業者ダッシュボードへ新規担当案件通知を追加
- [x] partner操作拒否・通知分離・一括処理・表示更新のテストを追加
- [x] Vitest・型チェック・実DBスモーク・本番ビルドを完走
- [x] StoreOSX mainと既存公開ドメインへ反映（5b4a63c、公開バンドル検証済み）

## 予定日集中警告／空き状況カレンダー（メール通知はユーザー指示により保留）
- [x] 既存の業者メール、通知送信、施工工程データを棚卸し
- [x] Resendによる新規割当・変更メール通知はユーザー指示により保留
- [x] メール未登録・送信失敗時の結果表示はメール通知再開時に実装
- [x] 同一予定日の案件集中件数を判定するAPIを実装
- [x] 管理者・社員の設定画面に予定日重複警告を追加
- [x] 業者別の施工スケジュール・空き状況APIを実装
- [x] 業者選択時に月間カレンダー形式で予定件数と案件を確認できるUIを追加
- [x] partnerの設定・他業者スケジュール取得を拒否する権限テストを追加
- [x] 集中判定・カレンダー表示のテストと実DBスモークを実行（業者候補38件・最大同日2件を確認）
- [x] 全Vitest・型チェック・本番ビルドを完走（34ファイル・362テスト）
- [x] StoreOSX mainと既存公開ドメインへ反映（f8afda4、公開バンドル検証済み）

## 実ユーザー権限再構成・コード確認資料・機能一覧スライド（ユーザー指示）
- [x] 三好・野口・新藤・河野・藤原の実ユーザーIDと現行ロールを確認
- [x] 最高管理者・管理者・社員・協力業者・顧客の権限マトリクスを確定
- [x] 最高管理者だけに利益・粗利を表示し、管理者はその他の管理機能・金額を利用可能にする
- [x] 社員は案件操作可能・社内金額非表示に統一
- [x] 協力業者は管理側指定エリアの案件だけ閲覧可能にし、提出見積・売上・原価・利益等を完全非表示
- [x] 管理画面から協力業者の閲覧エリアを設定できるUIを確認・補強
- [x] 三好＝最高管理者、野口＝管理者、新藤・河野・藤原＝社員へ実DBロールを更新
- [x] 役割切替・利益非表示・協力業者エリア制御・失注復活のテストを追加
- [x] 全Vitest・型チェック・実DBスモーク・本番ビルドを完走（35ファイル・368テスト）
- [x] StoreOSX mainと既存公開ドメインへ反映（ec875fb、公開バンドル検証済み）
- [x] 失注・復活の実装コードと役員／社員の画面テスト手順を技術資料に整理
- [x] システム機能一覧・権限管理・失注復活をまとめたスライドを作成

## 写真が見られない不具合の緊急修正（ユーザー指示・最優先）
- [x] 公開環境の写真表示エラーと対象画面を再現
- [x] 写真URL・ストレージ配信・認証・ブラウザエラーを診断
- [x] 案件詳細・報告書・ギャラリーの写真表示を修正
- [x] 既存写真と新規アップロード写真の表示を検証
- [x] 関連Vitest・型チェック・本番ビルドを実行
- [x] StoreOSX mainと既存公開ドメインへ緊急反映（3193579、正規・旧URLのJPEG取得を確認）

## 役員・社員の金額／エリア権限と失注・復活（ユーザー指示・優先）
- [x] 現行ロール・金額項目・案件エリア・顧客閲覧経路を棚卸し
- [x] 役員ロールとユーザー別閲覧エリア設定のスキーマ・マイグレーションを追加
- [x] 管理者は全情報・全エリア、役員は金額閲覧可、社員は社内金額非表示に統一
- [x] 協力業者・顧客は社内金額を非表示とし、公開承認済み指値・本人入力経費だけを例外表示
- [x] エリア未許可案件をAPIで返さないサーバー側制御を追加
- [x] 管理画面で役割と閲覧可能エリアを選択できるUIを追加
- [x] 失注状態・失注理由4区分・その他理由・失注前状態のスキーマを追加
- [x] 失注を完了相当として集計から除外し、再提出・復活時に即時復元できるAPIを追加
- [x] 予定日・業者設定欄へ失注ボタン、理由入力、復活ボタンを追加
- [x] 管理者・役員・社員・協力業者・顧客の権限テストを追加
- [x] 失注・復活・完了相当・再提出の回帰テストを追加
- [x] 全Vitest・型チェック・実DBスモーク・本番ビルドを完走（33ファイル・356テスト）
- [x] StoreOSX mainと既存公開ドメインへ反映（a800877、公開バンドル検証済み）

## 所感保存権限の統一（ユーザー指示）
- [x] 現行の所感保存API・入力画面・権限判定を確認
- [x] 最高管理者・管理者・社員の所感保存を許可
- [x] 協力業者・顧客・役員の所感保存とAI生成をAPIで拒否
- [x] 画面の所感入力・保存ボタンを最高管理者・管理者・社員だけに表示
- [x] 各ロールの保存可否をテスト（最高管理者・管理者・社員は許可、役員・協力業者・顧客は拒否）
- [x] 全Vitest・型チェック・本番ビルドを完走（36ファイル・377テスト）
- [x] StoreOSX mainと既存公開ドメインへ反映（01d2b32、公開CaseReportチャンク検証済み）

## 協力業者向け短文所感（ユーザー指示）
- [x] 既存の社内所感・協力業者メモ・案件アクセス構造を確認
- [x] 社内正式所感と分離した短文所感の保存項目を追加（既存partnerNotesを正式名称化）
- [x] 協力業者が担当・許可エリア案件だけへ短文所感を保存できるAPIを実装
- [x] 短文所感に文字数制限・保存者・保存日時を設定（200文字）
- [x] 協力業者向け入力・保存UIを追加（現調報告書・案件詳細）
- [x] 社内ロールから短文所感を閲覧可能にする
- [x] ロール・文字数・案件アクセスのテストを追加
- [x] 全Vitest・型チェック・本番ビルドを完走（36ファイル・380テスト）
- [x] StoreOSX mainと既存公開ドメインへ反映（eea41d1、公開CaseReport・CaseDetailチャンク検証済み）

## 短文所感の定型文・100文字化・社内未読通知（ユーザー指示）
- [x] 既存の短文所感と通知データ構造を確認
- [x] 短文所感の上限を200文字から100文字へ変更
- [x] よく使う定型文をワンクリック入力できるUIを追加
- [x] リアルタイム文字数を表示し80文字以上で警告色へ変更
- [x] 管理者・最高管理者・社員向け短文所感通知テーブル／APIを実装
- [x] ダッシュボードへ未読バッジ・新着通知・既読操作を追加
- [x] 役員・協力業者・顧客には社内通知を表示しない
- [x] 文字数・通知対象・既読・案件アクセスのテストを追加
- [x] 全Vitest・型チェック・本番ビルドを完走（37ファイル・384テスト）
- [x] StoreOSX mainと既存公開ドメインへ反映（7cfc0d9、公開バンドル検証済み）

## 現地調査報告書・完了報告書の出力形式改善提案（実装前確認）
- [x] 現行の現地調査報告書・完了報告書のページ構成とPDF生成方式を確認
- [x] 見栄え・読みやすさ・写真比較・承認判断・生成速度の評価軸を整理
- [x] 改善方向の異なる出力形式を5案作成し比較
- [x] 忖度のない推奨案と実装範囲を提示
- [x] ユーザー確認後に選択案を実装（案1＋完了報告のみ案2、サイン必須）

## 報告書改善案5案のビジュアル確認（実装前モックアップ）
- [x] 匿名化した共通案件データで比較条件を統一
- [x] 5案それぞれの現地調査・完了報告書A4モックアップを作成
- [x] 一覧比較画像と個別確認用PDFを作成
- [x] 見た目・可読性・ページ密度を確認して提示

## 統一標準レポート＋完了報告Before／After実装（ユーザー承認）
- [x] 現行の現地調査・完了報告レイアウト、写真分類、改ページ、PDF生成を再確認
- [x] 現地調査報告書を案1の統一標準レポート形式へ更新
- [x] 工事完了報告書の基本情報・見出し・表・フッターを統一標準形式へ更新
- [x] 工事完了報告書へ施工前／施工後の横並び比較を追加
- [x] 対応する写真がない場合は単独掲載し、写真を捏造しない
- [x] 現地調査報告書の電子サイン画像・署名者名・未署名時の手書き署名欄を維持
- [x] 工事完了報告書の電子サイン画像・署名者名・未署名時の手書き署名欄を維持
- [x] サイン登録・削除・PDFプレビュー・PDFダウンロードの既存動作を対象テストで回帰確認
- [x] レイアウト契約テスト、全Vitest、型チェック、本番ビルドを完走（39ファイル・392テスト）
- [x] A4プレビューで文字切れ・写真切れ・改ページ・サイン欄を視覚確認
- [x] GitHub StoreOSX mainと既存公開ドメインへ反映（e11ec31、公開バンドル検証済み）

## 報告書改善プロセスのスキル化＋編集・署名・プレビュー拡張（ユーザー指示）
- [x] 今回の提案・モックアップ・実装・PDF検証・公開プロセスを再利用可能なスキルとして作成
- [x] スキル未使用のベースラインシナリオを5回実行し、抜けやすい工程を記録
- [x] skill-creatorの初期化スクリプトで新規スキルを作成
- [x] 同一シナリオでスキル適用後の手順遵守を再検証
- [x] quick_validateでスキル形式を検証
- [x] Before／After写真の自動組み合わせと現行データ構造を確認
- [x] 案件ごとのBefore／After手動組み合わせ保存データ・APIを追加（ドラフトJSON・案件内写真ID検証・AI再生成時維持）
- [x] 完了報告書画面で写真の組み合わせを選択・入れ替え・解除できるUIを追加
- [x] 先方サインの署名者名・電子署名画像・署名日時を保存できるデータ・APIを追加（0040マイグレーション適用済み）
- [x] 画面上で先方が直接描画して保存・再署名・削除できる電子サインUIを追加
- [x] 現地調査・完了報告書PDFの先方確認欄へ保存済み先方サインを表示
- [x] 印刷・PDF出力前にA4全ページの実レイアウトを確認できるプレビューを追加（確認後のみ直接PDF出力可）
- [x] 写真組み合わせ・先方サイン・プレビュー・権限のテストを追加（5対象ファイル・28テスト）
- [x] 全Vitest・型チェック・本番ビルド・実画面PDF表示を検証（40ファイル・401テスト）
- [x] 作成スキルを検証して納品し、GitHub StoreOSX mainと既存公開ドメインへ反映（5e5855e、公開バンドル検証済み）

## 施工写真のAI自動分類（ユーザー指示）
- [x] 現行の写真区分・撮影日時・メモ・アップロード・権限制御を確認
- [x] 現調・施工前・施工中・施工後の判定基準と低確信度時の確認ルールを設計（80点未満は要確認）
- [x] 画像とメタデータを使って区分・確信度・理由を返すAI分類APIを追加（1回20枚、厳格JSON）
- [x] 分類候補を一覧で確認し、個別修正・除外できるプレビューUIを追加
- [x] 人の確認後だけ既存写真区分を一括保存し、確認前はDBを変更しない
- [x] owner・admin・user・許可案件のpartnerだけに分類・保存を許可
- [x] AI失敗・低確信度・不正な写真ID・権限外案件・既存データ非破壊のテストを追加（4ファイル・24テスト）
- [x] 全Vitest・型チェック・本番ビルド・実AI接続を検証（44ファイル・425テスト、候補取得時DB非変更）
- [x] GitHub StoreOSX mainと既存公開ドメインへ反映（7b9de6c、公開バンドル検証済み）

## AI写真分類の履歴・Undo（ユーザー指示）
- [x] 分類実行履歴に実行者・日時・変更前／変更後・確信度・理由を保存
- [x] 案件ごとの分類履歴一覧APIと閲覧権限を実装
- [x] 履歴単位で変更前区分へ一括復元するUndo APIを実装
- [x] Undo前に現在区分との不一致を検出し、後からの手動変更を保護
- [x] 二重Undo・後続分類競合・案件外写真・権限外・途中失敗の非破壊テストを追加（4ファイル・27テスト）
- [x] 写真タブへ履歴一覧・変更差分・Undo確認UIを追加
- [x] 全Vitest・型チェック・本番ビルド・実画面を検証（45ファイル・430テスト）
- [x] GitHub StoreOSX mainと既存公開ドメインへ反映（538fb36、公開バンドル検証済み）

## テスト協力業者・テスト案件の削除（ユーザー指示）
- [x] 名称・案件番号・メール等が明確にテスト用の候補を抽出（案件162件・協力業者160件）
- [x] 削除対象ごとの関連データ件数を確認し、実データを対象外に固定
- [x] テスト案件と関連データを依存順に削除
- [x] テスト協力業者と関連付けを安全に削除
- [x] 残存テスト候補ゼロと実案件442件・実協力業者39件の保持を確認

## 現場メモの配置提案（実装前確認）
- [x] 写真アップロード直後・写真一覧・案件詳細・報告書の現行導線を確認
- [x] 案件全体メモと写真個別メモの役割を整理
- [x] 入力者・閲覧者・金額非表示を含む権限方針を整理
- [x] 配置案を比較し、推奨構成と報告書への反映方法を提示
- [x] ユーザー確認後に実装範囲を確定（案2：写真タブ上部の案件全体メモ）

## 案件全体の現場メモ（案2・ユーザー承認）
- [x] 写真タブ・案件アクセス・既存監査情報の実装を確認
- [x] 案件単位の現場メモテーブルと記入者・日時・更新日時を設計（0042適用済み）
- [x] 現場メモの追加・一覧・本人編集・本人削除APIを追加
- [x] 最高管理者・管理者・社員と許可案件の協力業者だけが入力可能な権限を追加
- [x] 写真アップロード欄直下に時系列の現場メモ入力・一覧UIを追加
- [x] 既存の写真個別メモ・案件備考・正式所感を変更せず分離維持
- [x] 案件外アクセス・他人メモ編集・削除・ロール権限のテストを追加（2ファイル・12テスト）
- [x] 全Vitest・型チェック・本番ビルド・実画面表示を検証（47ファイル・443テスト）
- [x] GitHub StoreOSX mainと既存公開ドメインへ反映（7e3a022、公開写真タブチャンク検証済み）
