...
## 追加機能（v17）見積書一括取込＋個別案件収支

- [ ] estimates.extractAndMatch：PDF/画像をLLMに渡し、金額＋依頼番号/案件名/店舗名を抽出 → 既存案件とマッチ候補を返す
- [ ] estimates.bulkSave：複数見積書をまとめて保存（caseIdが確定したものだけ）
- [ ] /estimates/import ページ：複数ファイルD&D → AI抽出ローディング → マッチ確認テーブル（候補ドロップダウン・件名/金額/原価/作業費編集） → 一括コミット
- [ ] サイドバーに「見積書取込」リンクを追加
- [ ] 案件詳細に「収支」タブ：見積金額・予算（見積×75%）・原価合計（estimates合計）・実績・粗利・粗利率を表示
- [ ] vitest 4件以上：extractAndMatchバリデーション・bulkSave・候補マッチング
